...
> В случае если выдаёт ошибку `Host key verification failed` попробуйте повторить эти действия для основной ВМ, подключившись с неё к ВМ-копии. То есть запустите SHH для основной ВМ и выполните команду `ssh IP`, указав в качестве `IP`шника внутрнениий IP ВМ-копии.

Проверка подключения прошла успешно. Остался последний этап подготовок.

> Если на каком-то этапе возникли ошибки, перезапустите обе ВМ (сначала нажать **STOP**, потом **START**).
